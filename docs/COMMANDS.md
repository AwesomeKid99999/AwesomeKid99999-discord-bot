...
- **Permission Required:** None
- **Parameters:**
  - `[type]` (optional) - Display format: `classic`, `compact`, or `embed`. Defaults to `classic`.
  - `[limit]` (optional) - Number of users to display (1-100). Defaults to 10.
...
### `/poll <question> <choices> [embed]`
Create a poll with up to 20 options.
- **Permission Required:** None
- **Parameters:**
  - `<question>` (required) - The poll question
  - `<choices>` (required) - Comma-separated choices for the poll (max 20)
  - `[embed]` (optional) - Whether the poll should be an embed (true/false)
...
Enable or disable memes like "six seven" (brainrot) and "9+10=21".
- **Permission Required:** Manage Server
- **Parameters:**
