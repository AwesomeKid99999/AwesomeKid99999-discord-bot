...
exports.notInVoiceChannel = async ({ interaction, executionId }) => {
    ...
    if (!interaction.member.voice.channel) {
        ...
        console.debug(`User tried to use command '${interaction.commandName}' but was not in a voice channel.`);
        return true;
    }
...
exports.notInSameVoiceChannel = async ({ interaction, queue, executionId }) => {
    ...
    if (interaction.member.voice.channel.id !== queue.dispatcher.channel.id) {
        ...
        console.debug(`User tried to use command '${interaction.commandName}' but was not in the same voice channel.`);
        return true;
    }
